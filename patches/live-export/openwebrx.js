...
/* DNX_YELLOW_FOLLOW_CORE_START */
(function(){
  if(window.__dnxYellowFollowCore) return;
  window.__dnxYellowFollowCore = true;
  ...
  function styleOnce(){
    if(document.getElementById("dnx-yellow-follow-core-style")) return;
    var st = document.createElement("style");
    st.id = "dnx-yellow-follow-core-style";
    st.textContent = `
#dnx-core-yellow-line{
  ...
  background:linear-gradient(180deg,
    rgba(255,250,190,.20) 0%,
    rgba(255,236,120,.95) 12%,
    rgba(255,208,50,1) 44%,
    rgba(255,176,0,1) 72%,
    rgba(255,245,170,.16) 100%) !important;
  box-shadow:
    0 0 4px rgba(255,250,210,.56),
    0 0 10px rgba(255,220,80,.92),
    0 0 18px rgba(255,175,0,.46) !important;
}
#dnx-core-yellow-line::before{
  content:"" !important;
  ...
  box-shadow:0 0 4px rgba(255,255,255,.55) !important;
}
#dnx-core-yellow-glow{
  position:fixed !important;
  ...
  background:linear-gradient(90deg,
    rgba(255,200,0,0) 0%,
    rgba(255,240,150,.22) 50%,
    rgba(255,200,0,0) 100%) !important;
...
#dnx-core-yellow-cap::before{
  content:"" !important;
  position:absolute !important;
  inset:0 !important;
  background:linear-gradient(180deg,#fff27a 0%,#ffd93a 35%,#ffbf00 100%) !important;
  clip-path:polygon(22% 0,78% 0,100% 100%,0 100%) !important;
  border-radius:2px !important;
  box-shadow:0 0 4px rgba(255,245,150,.96),0 0 9px rgba(255,190,0,.74) !important;
}`;
    document.head.appendChild(st);
  }

  function killOtherLines(){
    [
      "dnx-follow-line","dnx-follow-line-glow","dnx-follow-topcap",
      "dnx-yf-line","dnx-yf-glow","dnx-yf-cap",
      "dnx-native-center-extend","dnx-native-center-extend-glow","dnx-center-topcap",
      "dnx-panel-tuned-line","dnx-panel-tuned-line-glow","dnx-panel-tuned-line-topcap",
      "dnx-real-center-line","owrx-smart-line-glow","dnx-wf-overlay"
    ].forEach(function(id){
      document.querySelectorAll("#"+id).forEach(function(el){ try{el.remove();}catch(e){} });
    });
    document.querySelectorAll(".dnx-sweep-bar,.dnx-center-line,.dnx-center-glow").forEach(function(el){
      try{el.remove();}catch(e){}
    });
  }

  function hide(){
    ["dnx-core-yellow-line","dnx-core-yellow-glow","dnx-core-yellow-cap"].forEach(function(id){
      var e = document.getElementById(id);
      if(e) imp(e, "display", "none");
    });
  }

  function yellowEnabled(){
    return window.__dnxYellowEnabled !== false;
  }
  ...
  function show(x){
    ...
    var scale = document.querySelector(".openwebrx-scale-container, #openwebrx-scale-container");
    var wf = document.querySelector(".openwebrx-waterfall-container, #openwebrx-waterfall-container");
    if(!scale || !wf) return;

    var sr = scale.getBoundingClientRect();
    var wr = wf.getBoundingClientRect();

    var line = ensure("dnx-core-yellow-line");
    var glow = ensure("dnx-core-yellow-glow");
    var cap  = ensure("dnx-core-yellow-cap");

    imp(cap,  "display", "block");
    imp(glow, "display", "block");
    imp(line, "display", "block");

    imp(cap,  "left", x + "px");
    imp(cap,  "top",  (sr.top + 4) + "px");

    imp(glow, "left", x + "px");
    imp(glow, "top",  (sr.bottom - 1) + "px");
    imp(glow, "height", (wr.bottom - sr.bottom + 3) + "px");

    imp(line, "left", x + "px");
    imp(line, "top",  (sr.bottom - 1) + "px");
    imp(line, "height", (wr.bottom - sr.bottom + 3) + "px");
  }
  ...
  function bind(){
    ...
    [scale, wf].forEach(function(el){
      if(!el || el.dataset.dnxCoreYellowBound === "1") return;
      el.dataset.dnxCoreYellowBound = "1";
      el.addEventListener("mousemove",  function(ev){
        if(!yellowEnabled()){ hide(); return; }
        show(ev.clientX);
      }, true);
  ...
  function boot(){
    ...
    killOtherLines();
    bind();
    setTimeout(function(){ killOtherLines(); bind(); }, 300);
    setTimeout(function(){ killOtherLines(); bind(); }, 1200);
  }
...
/* DNX_YELLOW_FOLLOW_CORE_END */


/* DNX_GREEN_EXTENSION_ADAPTIVE_START */
(function(){
  if(window.__dnxGreenExtensionAdaptive) return;
  window.__dnxGreenExtensionAdaptive = true;

  function clamp(v,a,b){ return Math.max(a, Math.min(b, v)); }
  function mix(a,b,t){ return [a[0]+(b[0]-a[0])*t, a[1]+(b[1]-a[1])*t, a[2]+(b[2]-a[2])*t]; }
  function rgb(c){ return "rgb(" + c.map(function(v){ return Math.round(v); }).join(",") + ")"; }
  function rgba(c,a){ return "rgba(" + c.map(function(v){ return Math.round(v); }).join(",") + "," + a + ")"; }
  ...
  function styleOnce(){
    if(document.getElementById("dnx-green-extension-adaptive-style")) return;
    var st = document.createElement("style");
    st.id = "dnx-green-extension-adaptive-style";
    st.textContent = `
#dnx-green-extension-adaptive-line{
  position:fixed !important;
  width:4px !important;
  transform:translateX(-50%) !important;
  pointer-events:none !important;
...
#dnx-green-extension-adaptive-line::before{
  content:"" !important;
  position:absolute !important;
  left:50% !important;
  top:0 !important;
...
#dnx-green-extension-adaptive-glow{
  position:fixed !important;
  width:10px !important;
  transform:translateX(-50%) !important;
  pointer-events:none !important;
  ...
  function hide(){
    ["dnx-green-extension-adaptive-line","dnx-green-extension-adaptive-glow"].forEach(function(id){
      var e = document.getElementById(id);
      if(e) imp(e, "display", "none");
    });
  }

  function findScaleCanvas(){
    return document.getElementById("openwebrx-scale-canvas")
      || document.querySelector(".openwebrx-scale-container canvas, #openwebrx-scale-container canvas");
  }

  function findGreenMarkerX(){
    var canvas = findScaleCanvas();
    ...
    var h = Math.min(canvas.height || 0, 40);
    ...
    for(var y=0; y<h; y++){
      for(var x=0; x<w; x++){
        ...
        if(g > 110 && g > r + 30 && g > b + 30){
          scores[x] += (g - Math.max(r, b));
        }
      }
    }

    var bestX = -1, bestScore = 0;
    for(var x=2; x<w-2; x++){
      var s5 = scores[x-2] + scores[x-1] + scores[x] + scores[x+1] + scores[x+2];
      if(s5 > bestScore){
        bestScore = s5;
        bestX = x;
      }
    ...
    if(bestX < 0 || bestScore < 260) return null;

    var rect = canvas.getBoundingClientRect();
    return rect.left + (bestX / w) * rect.width;
  }

  function findWaterfallCanvas(wf){
    var list = Array.from(wf.querySelectorAll("canvas")).filter(function(c){
  ...
  function sampleLumAtX(wf, xViewport){
    var c = findWaterfallCanvas(wf);
    if(!c) return null;

    var r = c.getBoundingClientRect();
    if(r.width < 10 || r.height < 10) return null;
    if(xViewport < r.left || xViewport > r.right) return null;

    var cx = Math.round((xViewport - r.left) / r.width * c.width);
    cx = clamp(cx, 2, c.width - 3);

    /* nur den frischen oberen Streifen lesen -> reagiert sofort */
    var y0 = Math.max(0, Math.round(c.height * 0.01));
    var y1 = Math.min(c.height, Math.round(c.height * 0.07));
    if(y1 <= y0) y1 = Math.min(c.height, y0 + 6);
    ...
    var data;
    try{
      data = ctx.getImageData(cx - 1, y0, 3, y1 - y0).data;
    }catch(e){
      return null;
    }
  ...
  function sampleLumAtPoint(wf, xViewport, yViewport){
    var c = findWaterfallCanvas(wf);
    if(!c) return null;
    ...
    if(r.width < 10 || r.height < 10) return null;
    if(xViewport < r.left || xViewport > r.right) return null;
    if(yViewport < r.top || yViewport > r.bottom) return null;

    var cx = Math.round((xViewport - r.left) / r.width * c.width);
    var cy = Math.round((yViewport - r.top) / r.height * c.height);
    cx = clamp(cx, 2, c.width - 3);
    cy = clamp(cy, 1, c.height - 2);
    ...
    var data;
    try{
      data = ctx.getImageData(cx - 1, cy, 3, 1).data;
    }catch(e){
      return null;
    }
  ...
  function ensureSeg(box, i, width){
    ...
    imp(el, "width", width + "px");
    return el;
  }

  function trimSegs(box, keep){
    for(var i=box.children.length-1; i>=keep; i--){
      box.children[i].style.display = "none";
    }
  }

  function paintGreenSegments(line, glow, wf, xViewport, top, height){
    imp(line, "background", "transparent");
    imp(line, "box-shadow", "none");
    imp(glow, "background", "transparent");
    imp(glow, "box-shadow", "none");

    var segH = 3;
    var count = Math.max(1, Math.ceil(height / segH));

    for(var i=0; i<count; i++){
      var y = top + i * segH;
      var realH = Math.max(1, Math.min(segH, top + height - y));
      var lum = sampleLumAtPoint(wf, xViewport, y + realH * 0.5);
      var dark = (lum == null) ? true : (lum < 90);
      var k = lum == null ? 0 : clamp((lum - 90) / 108, 0, 1);

      var topC  = dark ? [190,255,190] : mix([34,120,46],[18,82,28], k);
      var midC  = dark ? [110,255,110] : mix([20,98,34],[14,72,24], k);
      var deepC = dark ? [20,235,70]   : mix([10,78,28],[8,56,20], k);
      var glowC = dark ? [150,255,150] : mix([35,130,60],[18,82,28], k);

      var seg = ensureSeg(line, i, 4);
      imp(seg, "display", "block");
      imp(seg, "top", (y - top) + "px");
      imp(seg, "height", realH + "px");
      imp(seg, "background",
      ...
      imp(seg, "box-shadow",
        dark
          ? "0 0 5px " + rgba(glowC, 0.55) + ",0 0 10px " + rgba(glowC, 0.34)
          : "0 0 2px " + rgba(glowC, 0.22)
      );

      var gseg = ensureSeg(glow, i, 10);
      imp(gseg, "display", "block");
      imp(gseg, "top", (y - top) + "px");
      imp(gseg, "height", realH + "px");
      imp(gseg, "background",
        "linear-gradient(90deg,"
        + rgba(glowC, 0) + " 0%,"
        + rgba(glowC, dark ? 0.30 : 0.10) + " 50%,"
        + rgba(glowC, 0) + " 100%)"
      );
      imp(gseg, "filter", "blur(1.2px)");
      imp(gseg, "box-shadow", "none");
    }
  ...
  function applyAdaptive(line, glow, lum){
    /* hartes Umschalten statt weicher Mischung */
    var dark = (lum == null) ? true : (lum < 92);

    var top   = dark ? [190,255,190] : [34,120,46];
    var mid   = dark ? [110,255,110] : [20,98,34];
    var deep  = dark ? [20,235,70]   : [10,78,28];
    var shine = dark ? [245,255,245] : [170,220,180];
    var glowC = dark ? [150,255,150] : [35,130,60];
    ...
    imp(line, "box-shadow",
      dark
        ? "0 0 6px " + rgba(glowC, 0.55) + ",0 0 14px " + rgba(glowC, 0.38)
        : "0 0 3px " + rgba(glowC, 0.28) + ",0 0 8px " + rgba(glowC, 0.18)
    );

    imp(glow, "background",
      "linear-gradient(90deg,"
      + rgba(glowC, 0) + " 0%,"
      + rgba(glowC, dark ? 0.26 : 0.08) + " 50%,"
      + rgba(glowC, 0) + " 100%)"
    );

    var before = document.getElementById("dnx-green-extension-adaptive-before-style");
    if(!before){
      before = document.createElement("style");
      before.id = "dnx-green-extension-adaptive-before-style";
      document.head.appendChild(before);
    }
    before.textContent =
      "#dnx-green-extension-adaptive-line::before{background:linear-gradient(180deg,"
      + rgba(shine, dark ? 0.98 : 0.70) + ","
      + rgba(shine, dark ? 0.28 : 0.10) + ") !important;}";
  }
  ...
  function draw(){
    ...
    var line = ensure("dnx-green-extension-adaptive-line");
    var glow = ensure("dnx-green-extension-adaptive-glow");
    ...
    paintGreenSegments(line, glow, wf, x, (sr.bottom - 1), (wr.bottom - sr.bottom + 3));
  }
  ...
  function boot(){
    ...
    var __dnxGreenAdaptiveRaf = 0;
    function __dnxGreenAdaptiveLoop(){
      draw();
      __dnxGreenAdaptiveRaf = requestAnimationFrame(__dnxGreenAdaptiveLoop);
    }
    if(__dnxGreenAdaptiveRaf) cancelAnimationFrame(__dnxGreenAdaptiveRaf);
    __dnxGreenAdaptiveLoop();
...
/* DNX_GREEN_EXTENSION_ADAPTIVE_END */


/* DNX_YELLOW_ADAPTIVE_FOLLOW_START */
(function(){
  if(window.__dnxYellowAdaptiveFollow) return;
  window.__dnxYellowAdaptiveFollow = true;
  ...
  function neutralizeYellowCoreBefore(){
    var st = document.getElementById("dnx-yellow-segment-before-style");
    if(!st){
      st = document.createElement("style");
      st.id = "dnx-yellow-segment-before-style";
      document.head.appendChild(st);
    }
    st.textContent =
      "#dnx-core-yellow-line::before{background:transparent !important;box-shadow:none !important;}";
  }

  function paintYellowSegments(line, glow, wf, xViewport, top, height){
    neutralizeYellowCoreBefore();
    imp(line, "background", "transparent");
    imp(line, "box-shadow", "none");
    imp(glow, "background", "transparent");
    imp(glow, "box-shadow", "none");
    imp(glow, "filter", "none");
    ...
    for(var i=0; i<count; i++){
      ...
      var lum = sampleLumAtPoint(wf, xViewport, y + realH * 0.5);
      var dark = (lum == null) ? true : (lum < 92);
      var k = lum == null ? 0 : clamp((lum - 92) / 120, 0, 1);

      var topC  = dark ? [255,250,190] : mix([255,214,110],[170,120,20], k);
      var midC  = dark ? [255,220,85]  : mix([245,185,35],[140,95,15], k);
      var deepC = dark ? [255,185,0]   : mix([220,145,0],[110,70,10], k);
      var glowC = dark ? [255,220,90]  : mix([220,160,30],[120,85,20], k);

      var seg = ensureSeg(line, i, 3);
      imp(seg, "display", "block");
      imp(seg, "top", (y - top) + "px");
      imp(seg, "height", realH + "px");
      imp(seg, "background",
        "linear-gradient(180deg,"
        + rgba(topC, dark ? 0.20 : 0.16) + " 0%,"
        + rgb(midC) + " 45%,"
        + rgb(deepC) + " 70%,"
        + rgba(topC, dark ? 0.16 : 0.12) + " 100%)"
      );
      imp(seg, "box-shadow",
        dark
          ? "0 0 4px " + rgba(glowC, 0.56) + ",0 0 10px " + rgba(glowC, 0.42)
          : "0 0 2px " + rgba(glowC, 0.22)
      );
  ...
  function applyAdaptive(lum){
    var line = document.getElementById("dnx-core-yellow-line");
    var glow = document.getElementById("dnx-core-yellow-glow");
    var cap  = document.getElementById("dnx-core-yellow-cap");
    if(!line || !glow || !cap) return;

    var dark = (lum == null) ? true : (lum < 92);
    var t = lum == null ? 0 : clamp((lum - 92) / 120, 0, 1);

    var top   = dark ? [255,250,190] : mix([255,214,110],[170,120,20], t);
    var mid   = dark ? [255,220,85]  : mix([245,185,35],[140,95,15], t);
    var deep  = dark ? [255,185,0]   : mix([220,145,0],[110,70,10], t);
    var shine = dark ? [255,255,245] : mix([255,245,210],[185,145,70], t);
    var glowC = dark ? [255,220,90]  : mix([220,160,30],[120,85,20], t);

    imp(line, "background",
      "linear-gradient(180deg,"
      + rgba(top, dark ? 0.20 : 0.16) + " 0%,"
      + rgb(mid) + " 45%,"
      + rgb(deep) + " 70%,"
      + rgba(top, dark ? 0.16 : 0.12) + " 100%)"
    );

    imp(line, "box-shadow",
      dark
        ? "0 0 4px " + rgba(glowC, 0.56) + ",0 0 10px " + rgba(glowC, 0.42) + ",0 0 18px " + rgba(glowC, 0.24)
        : "0 0 2px " + rgba(glowC, 0.28) + ",0 0 6px " + rgba(glowC, 0.18) + ",0 0 10px " + rgba(glowC, 0.10)
    );

    imp(glow, "background",
      "linear-gradient(90deg,"
      + rgba(glowC, 0) + " 0%,"
      + rgba(glowC, dark ? 0.24 : 0.08) + " 50%,"
      + rgba(glowC, 0) + " 100%)"
    );

    var before = document.getElementById("dnx-yellow-adaptive-before-style");
    if(!before){
      before = document.createElement("style");
      before.id = "dnx-yellow-adaptive-before-style";
      document.head.appendChild(before);
    }
    before.textContent =
      "#dnx-core-yellow-line::before{background:linear-gradient(180deg,"
      + rgba(shine, dark ? 0.98 : 0.78) + ","
      + rgba(shine, dark ? 0.22 : 0.08) + ") !important;}";

    var capStyle = document.getElementById("dnx-yellow-adaptive-cap-style");
    if(!capStyle){
      capStyle = document.createElement("style");
      capStyle.id = "dnx-yellow-adaptive-cap-style";
      document.head.appendChild(capStyle);
    }
    capStyle.textContent =
      "#dnx-core-yellow-cap::before{"
      + "background:linear-gradient(180deg,"
      + rgb(mix(top,[255,255,220], dark ? 0.35 : 0.10)) + " 0%,"
      + rgb(mid) + " 35%,"
      + rgb(deep) + " 100%) !important;"
      + "box-shadow:"
      + (dark
          ? "0 0 4px " + rgba(glowC, 0.52) + ",0 0 9px " + rgba(glowC, 0.34)
          : "0 0 2px " + rgba(glowC, 0.24) + ",0 0 5px " + rgba(glowC, 0.12))
      + " !important;}";
  }
  ...
  function tick(){
    ...
    if(active && lastX !== null){
      var wf = qs(".openwebrx-waterfall-container, #openwebrx-waterfall-container");
      var scale = qs(".openwebrx-scale-container, #openwebrx-scale-container");
      var line = document.getElementById("dnx-core-yellow-line");
      var glow = document.getElementById("dnx-core-yellow-glow");
      if(wf && scale && line && glow){
        var sr = scale.getBoundingClientRect();
        var wr = wf.getBoundingClientRect();
        paintYellowSegments(line, glow, wf, lastX, (sr.bottom - 1), (wr.bottom - sr.bottom + 3));
      }
    }
    raf = requestAnimationFrame(tick);
  }
  ...
  function bind(){
    ...
    [scale, wf].forEach(function(el){
      if(!el || el.dataset.dnxYellowAdaptiveBound === "1") return;
      el.dataset.dnxYellowAdaptiveBound = "1";

      ["mousemove","mouseenter"].forEach(function(evname){
...
/* DNX_YELLOW_ADAPTIVE_FOLLOW_END */
